    ...
    while true; do
        ...
        if [ "$ufw_installed" -eq 0 ]; then
            ...
            if [ "$choice" -eq 0 ]; then
                ...
                (DEBIAN_FRONTEND=noninteractive apt-get install -y ufw 2>&1) > "$_ufw_log" &
        ...
        case $choice in
            0)
                ...
                echo -e "${BLUE}══════════════════════════════════════${NC}"
                printf "  ${DARKGRAY}%-5s   %-8s    %-4s     %-16s      %s${NC}\n" "№" "Порт" "Тип" "Состояние" "Комментарий"
                while IFS= read -r line; do
                    ...
                    printf "${WHITE}%-5s   %-6s   %-4s   ${state_color}%-16s${NC}   ${DARKGRAY}%s${NC}\n" "$idx" "$port_num" "$proto" "$state" "$comment"
